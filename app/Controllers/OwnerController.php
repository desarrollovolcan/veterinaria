...
class OwnerController extends BaseController
{
    ...
    private function handleSave(array &$errors): void
    {
        ...
        $data = [
            ...
            'telefono_movil' => trim((string) ($_POST['telefono_movil'] ?? '')),
            'telefono' => trim((string) ($_POST['telefono'] ?? '')),
            'email' => trim((string) ($_POST['email'] ?? '')),
            'direccion' => trim((string) ($_POST['direccion'] ?? '')),
            'ciudad' => trim((string) ($_POST['ciudad'] ?? '')),
            'identificacion' => trim((string) ($_POST['identificacion'] ?? '')),
            'idioma' => trim((string) ($_POST['idioma'] ?? 'Español')),
            'veterinario_derivante' => trim((string) ($_POST['veterinario_derivante'] ?? '')),
            'deuda' => trim((string) ($_POST['deuda'] ?? '')),
            'datos_facturacion' => trim((string) ($_POST['datos_facturacion'] ?? '')),
            'nombres_alternativos' => trim((string) ($_POST['nombres_alternativos'] ?? '')),
            'facebook' => trim((string) ($_POST['facebook'] ?? '')),
            'instagram' => trim((string) ($_POST['instagram'] ?? '')),
            'cumpleanos' => trim((string) ($_POST['cumpleanos'] ?? '')),
            'fuente_referencia' => trim((string) ($_POST['fuente_referencia'] ?? '')),
